inputString = """crruafyzloguvxwctqmphenbkd
...
for id in ids:
    ...
    for letter in id:
        if letter not in letterDic:
            letterDic.update({letter:1})
        else:
            val = letterDic.get(letter) + 1
            
            del letterDic[letter]
            letterDic.update({letter:val})
    
    values = letterDic.values()
    print(values)
    if 2 in values:
        twoCount += 1
        print("two increment")
    if 3 in values:
        print("threeIncrement")
        threeCount += 1
